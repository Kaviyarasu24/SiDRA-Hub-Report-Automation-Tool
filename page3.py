...
def generate_page3(excel_file, template_file, output_file):
    ...
    try:
        old_date = df['Old NDMI Date'].iloc[0].strip()  # Remove any whitespace or newlines
    ...
    try:
        ...
        current_date = df['Current NDMI Date'].iloc[0]
    ...
    if old_image_path:  # First box should have the old NDMI image
    ...
    if current_image_path:  # Second box should have the current NDMI image
        html_content = html_content.replace(
    ...
    html_content = html_content.replace('NDMI change', ndmi_change)
